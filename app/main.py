...
def api_search(q: str = Query(min_length=1), limit: int = Query(20, ge=1, le=100)):
    try:
        results, source = search_all(q, limit)
        return {"query": q, "total": len(results), "source": source, "results": results}
...
def home(q: str = ""):
    ...
    if q.strip():
        try:
            results, source = search_all(q.strip(), 50)
        except Exception as exc:
            error = str(exc)
    ...
    for r in results:
        ...
        href = r.get("source_url") or f"/judgment/{escape(r['jid'], quote=True)}"
        cards.append(f"""
        <article class='card'>
          <div class='meta'>{escape(r.get('date',''))} · {escape(r.get('jid',''))}</div>
          <h2><a href='{escape(href, quote=True)}' target='_blank' rel='noopener'>{escape(r.get('title') or '裁判書')}</a></h2>
          <div class='label'>被告人</div>
          <div class='people'>{people}</div>
          <div class='snippet'>{escape(r.get('snippet') or '')}</div>
        </article>
        """)
...
.card{{background:white;border:1px solid #ddd;border-radius:10px;padding:18px;margin:12px 0}}h2{{font-size:18px;margin:8px 0}}a{{color:#174ea6;text-decoration:none}}.meta{{font-size:13px;color:#777}}.snippet{{line-height:1.7;color:#444;margin-top:12px;white-space:pre-wrap}}.error{{color:#b00020}}.label{{font-weight:700;margin-top:12px}}.people{{display:flex;gap:7px;flex-wrap:wrap;margin-top:7px}}.person{{background:#fff0f0;border:1px solid #e5b4b4;border-radius:999px;padding:5px 10px}}.none{{color:#888}}.source{{font-size:13px;color:#666;margin:8px 0 16px}}
</style></head><body>
...
{f"<p class='source'>{escape(source_text)}</p>" if source_text else ''}
